...
Live-verified 2026-07-22, now 4 of the 4-pass standard (passes 3-4 completed in a second, independent session the same day): (1) both the devnet default build and `cargo test --features mainnet` ran 52/52 clean, after rebuilding `target/deploy-mainnet/plotarmor.so`, which had gone stale since the 2026-07-03 Finding 1 deploy and did not contain the `sign_contract` instruction — 6 of 7 `sign_contract_tests.rs` cases failed with `InstructionFallbackNotFound` against the stale binary before the rebuild (build-artifact staleness, not a program-logic defect); (2) `anchor test` (TypeScript/Mocha, local validator — a distinct execution harness from LiteSVM) ran 34/34 clean on first run, including the 4 new `sign_contract` tests below; (3) independent re-audit: `sign_contract.rs`/`state.rs`/`error.rs` re-read fresh against `sign_contract-spec.md` in a separate session, confirming the `ContractSignature` layout, `LEN` (121 bytes), and `ContentHashMismatch`-only error surface match the spec exactly, plus an independent re-run of both cargo test builds (52/52 each) and `anchor test` (34/34); (4) live devnet adversarial check via new script `scripts/devnet_sign_contract_adversarial_test.ts` — wrong `content_hash` → `ContentHashMismatch` (6011), double-sign by the same signer → already-in-use, signing a nonexistent `ContractArtifact` → account-validation failure, all 3 passed on first live run against the deployed devnet program.
...
- `sign_contract` Layer 2 coverage (added 2026-07-22): 1 happy path (creates `ContractSignature`, asserts `contractArtifact`/`signer`/`contentHash` match the submitted values and `signedAt`/`slot` are populated from the chain) + 3 rejections mirroring `sign_contract_tests.rs` — wrong `content_hash` → `ContentHashMismatch` (6011), double-sign by the same signer → account already in use, signing a nonexistent `ContractArtifact` → account-validation failure. Adversarial live-devnet coverage for these same three paths was added the same day via `scripts/devnet_sign_contract_adversarial_test.ts` — see §10.
...
| 6011 | `ContentHashMismatch` | Added 2026-07-15 with `sign_contract`. LiteSVM: `wrong_content_hash_rejected` in `sign_contract_tests.rs`. Exercised live on devnet 2026-07-22 via `scripts/devnet_sign_contract_adversarial_test.ts`. |

System Program `0x0` ("already in use") confirmed on: duplicate `OwnerRecord`, evidence replay, authorized contract PDA collision, `init_registry_config` second call, and (as of 2026-07-15) double-sign of the same `ContractSignature` PDA (`double_sign_by_same_signer_rejected`, LiteSVM; confirmed live on devnet 2026-07-22 via `scripts/devnet_sign_contract_adversarial_test.ts`).
